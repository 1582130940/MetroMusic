...
class CoverLrcView @JvmOverloads constructor(
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        mCurrentTextSize = ta.getDimension(
            /* index = */ R.styleable.LrcView_lrcTextSize, /* defValue = */
    ...
    fun updateTime(time: Long) {
        runOnUi {
            ...
            val line = findShowLine(time = time + 300L)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.translate(/* dx = */ 0f, /* dy = */ mOffset)
        var y = 0f
    ...
    private fun drawText(canvas: Canvas, staticLayout: StaticLayout, y: Float) {
        canvas.withSave {
            translate(/* dx = */ mLrcPadding, /* dy = */ y - (staticLayout.height shr 1))
            staticLayout.draw(/* c = */ this)
        }
    }
    ...
    fun reset() {
        endAnimation()
        mScroller?.forceFinished(/* finished = */ true)
        isShowTimeline = false
        isTouching = false
        isFling = false
        removeCallbacks(hideTimelineRunnable)
