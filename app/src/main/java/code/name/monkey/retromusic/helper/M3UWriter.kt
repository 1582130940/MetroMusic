...
object M3UWriter : M3UConstants {
    ...
    fun write(
        ...
        val file =
            File(/* parent = */ dir, /* child = */ playlist.name + "." + M3UConstants.EXTENSION)
        ...
        if (songs.isNotEmpty()) {
            BufferedWriter(FileWriter(file)).use { bw ->
                ...
                for (song in songs) {
                    bw.newLine()
                    bw.write(/* str = */ M3UConstants.ENTRY + song.duration + M3UConstants.DURATION_SEPARATOR + song.artistName + " - " + song.title)
                    bw.newLine()
                    bw.write(song.data)
                }
            }
    ...
    fun writeIO(dir: File, playlistWithSongs: PlaylistWithSongs): File {
        ...
        if (songs.isNotEmpty()) {
            BufferedWriter(FileWriter(file)).use { bw ->
                ...
                songs.forEach {
                    bw.newLine()
                    bw.write(/* str = */ M3UConstants.ENTRY + it.duration + M3UConstants.DURATION_SEPARATOR + it.artistName + " - " + it.title)
                    bw.newLine()
                    bw.write(it.data)
                }
            }
