...
object RingtoneManager {
    fun setRingtone(context: Context, song: Song) {
        ...
        try {
            val cursor =
                resolver.query(/* uri = */ MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, /* projection = */
                    arrayOf(MediaStore.MediaColumns.TITLE), /* selection = */
                    BaseColumns._ID + "=?", /* selectionArgs = */
                    arrayOf(song.id.toString()), /* sortOrder = */
                    null
                )
